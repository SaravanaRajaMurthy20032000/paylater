...
async def transact(transaction: TransactionCreate):
    ...
    try:
        ...
        if balance >= transaction.amount:
            ...
            query = f"INSERT INTO transaction (u_id, m_id, amount) VALUES ({transaction.u_id}, {transaction.m_id}, {transaction.amount})"
            cursor.execute(query)
            conn.commit()
...
async def get_user_dues(user: str):
    ...
    try:
        conn = create_connection()
        cursor = conn.cursor()
        query = f"SELECT SUM(amount) FROM transaction WHERE u_id=(SELECT user_id FROM user WHERE name='{user}')"
        cursor.execute(query)
